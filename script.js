...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- AUTHENTICATION FLOW (INSTANT UI UPDATE) ---
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            // 1. INSTANTLY update the UI to prevent freezing
            ...
            // 2. Silently update database in background (does not block UI if it fails)
            setDoc(doc(db, "users", user.uid), {
                name: user.displayName,
                email: user.email,
                lastLogin: serverTimestamp()
            }, { merge: true }).catch(err => console.warn("Firestore not initialized yet:", err));
