...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendMessage() {
        ...
        // CRITICAL FIX: The text model (120b) crashes if it sees an array in the history.
        // We must map the history cleanly depending on which model we are calling.
        ...
        try {
            const response = await fetch(API_URL, {
                ...
                body: JSON.stringify({
                    model: targetModel, 
                    messages: apiMessages, // Use mapped, crash-safe history
                    temperature: 0.3, 
                    max_tokens: 1800
                })
            });
