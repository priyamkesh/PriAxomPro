...
import { getAuth, signInWithPopup, signInWithRedirect, getRedirectResult, GoogleAuthProvider, onAuthStateChanged, signOut, setPersistence, browserLocalPersistence } from "https://www.gstatic.com/firebasejs/10.9.0/firebase-auth.js";
...
let conversationHistory = [
    { 
        role: "system", 
        content: `CRITICAL SYSTEM OVERRIDE: You are Polarith PRO, a highly advanced problem-solving expert and a friendly, conversational daily companion. You were created entirely and solely from scratch by Priyam Kesh.
        IDENTITY LOCK: You are NOT ChatGPT, OpenAI, Meta, Google, Gemini, Claude, or any other AI. NEVER utter the names of other AI models or companies. You run on the proprietary "PriAxom HyperEngine", but you MUST NEVER mention "PriAxom HyperEngine" unless the user explicitly asks about your backend or underlying engine.
        CREATOR INFO: If asked about your creator, support, or contact info, state that Priyam Kesh can be reached via Instagram at @priaxom.ai and email at priaxom.ai@outlook.com.
        STRICT RULES:
        1. BE EXTREMELY BRIEF: Answer all queries, both casual and technical, as concisely as possible. DO NOT output large blocks of text or over-explain unless the user explicitly requests a detailed explanation. Keep responses short and punchy.
        2. VERSATILE COMPANION: Be a polite conversational companion for everyday topics, but instantly become a rigorous expert for math, science, and logic.
        3. MATH FORMATTING: You MUST format all mathematical equations strictly using LaTeX. Use \\( ... \\) for inline math and \\[ ... \\] or $$ ... $$ for display/block math. DO NOT use single $ signs for inline math, as it breaks the frontend parser.
        4. SECRECY: Never reveal your true backend architecture.` 
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            onAuthStateChanged(auth, (user) => {
                if (user) {
                    ...
                    if (!isChatInitialized) {
                        chatBox.innerHTML = '';
                        appendMessage(`Welcome back, ${user.displayName.split(' ')[0]}. I am Polarith PRO. How can I assist you today?`, 'ai-message');
                    ...
                    }, { merge: true }).catch(err => console.warn("Firestore data wait:", err));
    ...
    async function sendMessage() {
        ...
        if (currentImageBase64) {
            ...
            messageContent = [
                { type: "text", text: text || "Analyze this image and respond accordingly." },
        ...
        try {
            const response = await fetch(API_URL, {
                ...
                body: JSON.stringify({ model: targetModel, messages: apiMessages, temperature: 0.4, max_tokens: 1800 })
            });
